...
if(isset($_POST['add'])){
    ...
    if($db->insert_prislusenstvi($nazev)){
        $goodMsg = "příslušenství úspěšně přidáno!";
    }else{
...
if(isset($_POST['delete'])){
    if($db->delete_prislusenstvi($_POST['id'])){
        $goodMsg = "příslušenství úspěšně odebráno!";
    }else{
        $errorMsg = "něco se nepovedlo :(";
    }
}
...
if(isset($_POST['update'])){
    if($db->update_prislusenstvi($_POST['id'],htmlspecialchars($_POST['nazev']))){
        $goodMsg = "příslušenství úspěšně upraveno :)";
    }else{
        $errorMsg = "něco se nepovedlo :(";
    }
}
...
<div class="table-responsive shadow-lg mt-3" id="responsive-table">
    <table class="tabulka ms-auto me-auto text-center">
        ...
        if (isset($_GET["nazev"]) && $_GET['nazev'] != "") {
            ...
            $pom = array();
            foreach ($viewPrislusenstvi as $item) { // filtr mistnosti
                if (!is_bool(strpos(strtolower($item["NAZEV"]), strtolower($value), 0))) {
                    array_push($pom, $item);
                }
            }
            $viewPrislusenstvi = $pom;
        }


        //tvorba tabulky
        if (count($viewPrislusenstvi) == 0) {
            echo "<tr><td colspan='10'>Nebyl nalezen žádný</td></tr>";
        } else {
            foreach ($viewPrislusenstvi as $item) {
             ...
             <button class="btn btn-light text-uppercase p-0 " type="button" data-bs-toggle="collapse"
                data-bs-target="#item' . $item['ID_PRISLUSENSTVI'] . '"  aria-expanded="false" aria-controls="item' . $item['ID_PRISLUSENSTVI'] . '"><span class="material-symbols-outlined">edit</span>
            </button>
                  </td>';
                }
                ...
                if (isset($_SESSION['ROLE']) && $_SESSION['ROLE'] == 1) {
                    echo '<tr class="radek-edit text-start"><td colspan="10" class="p-0"><div class="collapse" id="item' . $item['ID_PRISLUSENSTVI'] . '">
<form class="w-100 px-2" action="" method="post">
<div class="row">
<input name="id" class="w-100 d-none" type="text" value="' . $item['ID_PRISLUSENSTVI'] . '" required readonly>
<div><label>název:</label><input name="nazev" class="w-100" type="text" value="' . $item['NAZEV'] . '" required id="nazev' . $item['ID_PRISLUSENSTVI'] . '"></div>
...
<input class="d-none" type="text" name="id" value="' . $item['ID_PRISLUSENSTVI'] . '">
<button class="btn btn-danger" type="submit" name="delete">Delete</button>
</form>
</div></td></tr>';
                    echo '<script>document.getElementById("nazev' . $item['ID_PRISLUSENSTVI'] . '").value = ' . $item['NAZEV'] . '</script>'; // nastavení oprávnění na aktouální hodnotu
                }
            }
        }
        ?>
