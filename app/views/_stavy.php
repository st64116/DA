...
if(isset($_POST['update'])){
    if($db->update_stav($_POST['id'],htmlspecialchars($_POST['nazev']))){
...
<div class="table-responsive shadow-lg mt-3" id="responsive-table">
    <table class="tabulka ms-auto me-auto text-center">
        ...
        if (isset($_GET["nazev"]) && $_GET['nazev'] != "") {
            ...
            $pom = array();
            foreach ($viewStavy as $item) { // filtr mistnosti
        ...
        if (count($viewStavy) == 0) {
            ...
            foreach ($viewStavy as $item) {
             ...
             <button class="btn btn-light text-uppercase p-0 " type="button" data-bs-toggle="collapse"
                data-bs-target="#item' . $item['ID_STAVU'] . '"  aria-expanded="false" aria-controls="item' . $item['ID_STAVU'] . '"><span class="material-symbols-outlined">edit</span>
            </button>
                  </td>';
                }
                ...
                if (isset($_SESSION['ROLE']) && $_SESSION['ROLE'] == 1) {
                    echo '<tr class="radek-edit text-start"><td colspan="10" class="p-0"><div class="collapse" id="item' . $item['ID_STAVU'] . '">
<form class="w-100 px-2" action="" method="post">
<div class="row">
<input name="id" class="w-100 d-none" type="text" value="' . $item['ID_STAVU'] . '" required readonly>
<div><label>název:</label><input name="nazev" class="w-100" type="text" value="' . $item['NAZEV'] . '" required id="nazev' . $item['ID_STAVU'] . '"></div>
...
<input class="d-none" type="text" name="id" value="' . $item['ID_STAVU'] . '">
<button class="btn btn-danger" type="submit" name="delete">Delete</button>
</form>
</div></td></tr>';
                    echo '<script>document.getElementById("nazev' . $item['ID_STAVU'] . '").value = ' . $item['NAZEV'] . '</script>'; // nastavení oprávnění na aktouální hodnotu
                }
            }
        }
        ?>
