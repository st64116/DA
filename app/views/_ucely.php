...
if(isset($_POST['update'])){
    if($db->update_ucel($_POST['id'],$_POST['nazev'])){
        ...
        if (count($viewUcely) == 0) {
            ...
            foreach ($viewUcely as $item) {
             ...
             <button class="btn btn-light text-uppercase p-0 " type="button" data-bs-toggle="collapse"
                data-bs-target="#item' . $item['ID_UCELU'] . '"  aria-expanded="false" aria-controls="item' . $item['ID_UCELU'] . '"><span class="material-symbols-outlined">edit</span>
            </button>
                  </td>';
                }
                ...
                if (isset($_SESSION['ROLE']) && $_SESSION['ROLE'] == 1) {
                    echo '<tr class="radek-edit text-start"><td colspan="10" class="p-0"><div class="collapse" id="item' . $item['ID_UCELU'] . '">
<form class="w-100 px-2" action="" method="post">
<div class="row">
<input name="id" class="w-100 d-none" type="text" value="' . $item['ID_UCELU'] . '" required readonly>
<div><label>název:</label><input name="nazev" class="w-100" type="text" value="' . $item['NAZEV'] . '" required id="nazev' . $item['ID_UCELU'] . '"></div>
...
<input class="d-none" type="text" name="id" value="' . $item['ID_UCELU'] . '">
<button class="btn btn-danger" type="submit" name="delete">Delete</button>
</form>
</div></td></tr>';
                    echo '<script>document.getElementById("nazev' . $item['ID_UCELU'] . '").value = ' . $item['NAZEV'] . '</script>'; // nastavení oprávnění na aktouální hodnotu
                }
            }
        }
        ?>
